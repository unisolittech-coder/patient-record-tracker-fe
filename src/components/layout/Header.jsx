      ...
      <div className="flex items-center gap-6">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            ...
            {userName && <span className="text-xs text-gray-500">{userRole === 'super_admin' ? 'Super Admin' : userRole === 'receptionist' ? 'Receptionist' : userRole === 'doctor' ? 'Doctor' : userRole === 'payment_officer' ? 'Payment Officer' : userRole === 'blood_collection_officer' ? 'Blood Collection Operator' : userRole}</span>}
          </div>
        </div>
      </div>
    </header>
