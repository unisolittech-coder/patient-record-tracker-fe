...
export default function Sidebar() {
  ...
  return (
    <aside
      className={`${isCollapsed ? "w-20" : "w-72"} transition-all duration-300 bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900 text-white flex flex-col h-[calc(100vh-4rem)] sticky top-16 shadow-2xl border-r border-white/10 relative`}
    >
      {/* Toggle Button */}
      <button
      ...
      >
        <i
          className={`pi ${isCollapsed ? "pi-angle-right" : "pi-angle-left"} text-xs`}
        ></i>
      </button>
      ...
      <div className="p-4 border-t border-white/10 bg-white/5 backdrop-blur-sm overflow-hidden">
        <div
          className={`flex items-center ${isCollapsed ? "justify-center flex-col gap-4" : "justify-between"}`}
        >
